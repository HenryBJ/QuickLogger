...
public class LogRepository : IRepository<Log, Guid>
{
    ...
    {
        return await Task.FromResult(_context.Logs.Where(predicate));
    }
    ...
    {
        return await _context.Logs.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
    }
    ...
    {
        var query = ascending ? _context.Logs.OrderBy(keySelector) : _context.Logs.OrderByDescending(keySelector);
        return Task.FromResult(query.Skip((pageNumber - 1) * pageSize).Take(pageSize));
    }
    ...
    public Task<IEnumerable<Log>> SearchWithPagingAsync(Func<Log, bool> predicate, int pageNumber, int pageSize)
    {
        var filteredLogs = _context.Logs.Where(predicate);
        return Task.FromResult(filteredLogs.Skip((pageNumber - 1) * pageSize).Take(pageSize));
    }
