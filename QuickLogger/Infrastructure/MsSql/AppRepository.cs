...
public class AppRepository : IRepository<App, Guid>
{
    ...
    public  Task<int> CountAsync(Func<App, bool> predicate)
    {
        return Task.FromResult(_context.Apps.Where(predicate).Count());   
    }
    ...
    {
        return await _context.Apps.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToListAsync();
    }

    public async Task<App?> GetSingleAsync(Func<App, bool> predicate)
    {
        return await Task.FromResult(_context.Apps.SingleOrDefault(predicate));
    }
    ...
    {
        return await Task.FromResult(_context.Apps.Where(predicate).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList());
    }
