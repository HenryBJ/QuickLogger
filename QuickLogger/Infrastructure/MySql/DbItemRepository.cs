...
public class DbItemRepository : IRepository<DBItem, Guid>
{
    ...
    {
        await _context.DBItems.AddAsync(entity);
    ...
    {
        return await Task.FromResult(_context.DBItems.Count(predicate));
    }
    ...
    {
        return await _context.DBItems.AnyAsync(db => db.Id == id);
    }

    public async Task<bool> ExistsAsync(Func<DBItem, bool> predicate)
    {
        return await Task.FromResult(_context.DBItems.Any(predicate));
    }

    public async Task<IEnumerable<DBItem>> FindAsync(Func<DBItem, bool> predicate)
    {
        return await Task.FromResult(_context.DBItems.Where(predicate).ToList());
    }
    ...
    {
        var query = ascending
            ? _context.DBItems.OrderBy(keySelector)
            : _context.DBItems.OrderByDescending(keySelector);
    ...
    public async Task<IEnumerable<DBItem>> SearchWithPagingAsync(Func<DBItem, bool> predicate, int pageNumber, int pageSize)
    {
        return await Task.FromResult(
            _context.DBItems.Where(predicate)
                .Skip((pageNumber - 1) * pageSize)
                .Take(pageSize)
                .ToList()
        );
